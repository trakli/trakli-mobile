...
/// Insights screen for a single party. Mobile-native composition: a tonal
/// hero card that drives the page palette, a totals row, a 6-month activity
/// chart, and a list of recent transactions involving this party. No
/// transaction-fetching logic of its own — reads from the existing
/// TransactionCubit so other flows stay untouched.
class PartyDetailScreen extends StatelessWidget {
  ...
  ({double received, double spent, double net}) _totals(
    ...
    double spent = 0;
    for (final t in txns) {
      if (t.party?.clientId != party.clientId) continue;
  ...
  List<TransactionCompleteEntity> _recent(
    ...
    final mine = txns
        .where((t) => t.party?.clientId == party.clientId)
        .toList()
      ..sort(
  ...
  List<_MonthBucket> _monthly(List<TransactionCompleteEntity> txns) {
    ...
    for (final t in txns) {
      if (t.party?.clientId != party.clientId) continue;
      final key = DateTime(t.transaction.datetime.year,
          t.transaction.datetime.month, 1);
      ...
      builder: (context, state) {
        ...
        return Scaffold(
          ...
          body: ListView(
            ...
            children: [
              _HeroCard(party: party, tone: heroTone),
...
class _HeroCard extends StatelessWidget {
  ...
  const _HeroCard({required this.party, required this.tone});
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                if (party.type != null || party.description?.isNotEmpty == true) ...[
                  SizedBox(height: 2.h),
                  Row(
                    children: [
                      if (party.type != null)
                      ...
                      if (party.type != null &&
                          party.description?.isNotEmpty == true) ...[
                        SizedBox(width: 6.w),
                        Container(
                          width: 2,
                          height: 2,
                      ...
                      if (party.description?.isNotEmpty == true)
                        Flexible(
                          child: Text(
                            party.description!,
                            style: TextStyle(
...
class _ActivityCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final tones = context.tones;
    final hasData =
        monthly.any((m) => m.received > 0 || m.spent > 0);
            ...
            SizedBox(
              ...
              child: SfCartesianChart(
                ...
                primaryXAxis: CategoryAxis(
                  ...
                  axisLine:
                      AxisLine(width: 0.5, color: tones.borderLight),
