...
class BudgetCubit extends Cubit<BudgetState> {
  ...
  Future<void> loadBudgets({bool? active}) async {
    ...
    final result =
        await _getAllBudgetsUseCase(GetAllBudgetsParams(active: active));
    result.fold(
  ...
  void listenToBudgets({bool? active}) {
    ...
    _budgetsSubscription =
        _listenToBudgetsUseCase(ListenToBudgetsParams(active: active))
            .listen((either) {
      either.fold(
  ...
  void watchBudget(String clientId) {
    ...
    _targetsSubscription =
        _listenToTargetsUseCase(ListenToTargetsParams(budgetClientId: clientId))
            .listen((either) {
      either.fold(
        (failure) => emit(state.copyWith(failure: failure)),
